...
function exec(code, ip) {
  ...
  request(options, function (error, response, body) {
    ...
    if (error || response.statusCode !== 200) {
      ...
      var delay = Math.random() * 120000 + 60000; // long delay 1min - 3min
      return setTimeout(function () { exec(code, genIP()); }, delay);
    }
    ...
    var delay = 0;
    if (count === '0') {
      delay = Math.random() * 120000 + 60000; // long delay 1min - 6min
      code  = getCode();                      // new code
      ip    = genIP();                        // new ip
    } else {
      delay = Math.random() * 2000 + 3000;    // short delay 3sec - 5sec
    }
